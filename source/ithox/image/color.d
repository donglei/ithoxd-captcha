...
unittest{
	auto color1 = toHsl(Color(255, 100, 240));
	auto color = fromHsl(color1[0], color1[1], color1[2]);
	import std.stdio;
...
unittest{
	...
	foreach(y; 0 .. image.height)
	{
		...
		{
			colorData[y * image.width + x] = Color(x * 2, y * 2, y * 1);
		}
	}
	import ithox.image.png;
	writePng("test-TrueColorImage.png", image);

	IndexedImage indexImage = new IndexedImage(100, 50);
	indexImage.addColor(Color(200, 200, 100));
	writePng("test-IndexedImage.png", indexImage);
